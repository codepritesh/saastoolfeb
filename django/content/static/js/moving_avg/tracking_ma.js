var alias = bot_alias;
// var srv_id = "{{ srv_id|escapejs }}";
console.log('server_id: ', srv_id)
console.log('bot_alias: ', bot_alias)
var trackingSocket = new WebSocket('ws://' + window.location.host +'/ws/ma/' + alias + srv_id +'/tracking');
	...
	var data = datas['message'];
        ...
        for (item in data) {
            ...
            let append_html = `<tr id=${abot.uuid}>\
                                 <td><a href="/ma-bot/${abot.uuid}" target="_blank">${abot.uuid}</a></td>\
                                 <td>${abot.time}</td>\
                                 <td>${abot.own_name}</td>\
                                 <td>${abot.pair}</td>\
                               </tr>`;
