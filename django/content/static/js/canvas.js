...
(function() {
 ...
 var $a = {},
  ...
  ea = function(h, p, r, u, v, H, F, z, E, L, R, N, O) {
   ...
   h.globalAlpha = 1;
   0 < F && (O = 0 === F % 2 ? 0 : 0.5, h.beginPath(), h.lineWidth = F, h.moveTo(p, r), h.rect(p - O, r - O, u - p + 2 * O, v - r + 2 * O), h.stroke());
   h.restore();
 ...
 Na.Chart = function() {
  ...
  p.prototype.renderCandlestick = function(a) {
   ...
   if (!(0 >= a.dataSeriesIndexes.length)) {
    ...
    b.beginPath();
    b.setLineDash([1, 1]);
    b.moveTo(k.x1, last_close);
    b.lineTo(k.x2, last_close);
    b.stroke();
    r && (e = N(aa), c.strokeStyle = e, c.setLineDash([1, 1]), c.beginPath(), c.moveTo(k.x1, last_close), c.moveTo(k.x2, last_close), c.stroke())
    for (const i of open_orders) {
	var color;
	var price = a.axisY.convertValueToPixel(i.p);
	b.beginPath();
	if (i.d == 'buy') {
	    color = '#33ff33'
	} else {
	...
	b.strokeStyle = color
        ...
        b.lineTo(k.x2, price);
        b.stroke();
        r && (e = N(aa), c.strokeStyle = color, c.setLineDash([6, 6]), c.beginPath(), c.moveTo(k.x1, price), c.moveTo(k.x2, price), c.stroke())
    }
