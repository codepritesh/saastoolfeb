...
import requests
import traceback
from tool_atr_trailing_stop import *
from threading import Thread
import json

class AtrTrailingStopToolConsumer(BaseConsumer):
    BOT_ALIAS = AtrTrailingStopTool.bot_alias
    ...
    def receive(self, text_data):
        ...
        if signal == 'start':
            try:
                ...
                print('ERROR: AtrTrailingStopToolConsumer: {}'.format(tb))
                return False
        if signal == 'stop':
            try:
    ...
    def stop_bot(self, web_input):
        try:
            ...
            tb = traceback.format_exc()
            print("AtrTrailingStopToolConsumer: ", tb)
        return True
