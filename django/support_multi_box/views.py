...
from .consumers import SupportMultiBoxConsumer
...
def index(request, id=''):
    ...
    context = {'apis': apis, 'user': us, 'bot_alias': SupportMultiBoxConsumer.BOT_ALIAS, 'ws_define': 'ws_support_multi_box'}
    if id:
        base_srv = BaseService()
        bot_alias = SupportMultiBoxConsumer.BOT_ALIAS
        data = base_srv.fetch_detail(bot_alias, str(id))
        if data:
            apis = [data.get('api_name')]
            context.update({'apis': apis, 'id': id, 'user': us, 'data': data, 'bot_alias': SupportMultiBoxConsumer.BOT_ALIAS})
    ...
    if user_permission_data.support_multi_box:
        try:
        ...
        return render(request, 'support_multi_box/index.html', context)
    else:
        messages.error(request, ('You are not allowed to see support_multi_box page please subscribe'))
...
def tracking(request):
    ...
    data = {'tracking_user':tracking_user,'srv_id': base_srv.srv_id, 'bot_alias': SupportMultiBoxConsumer.BOT_ALIAS, 'suff_ws': 'ws_support_multi_box',
            'bot_name': 'Support Multi Boxes Bot', 'title': 'Support Multi Boxes Bot', 'bot_url': 'support-multi-box'}
    try:
        user_id = request.user.id  
        UserBotInstancesRunning_form = UserBotInstancesRunning.objects.get(pk=user_id)
        bot_one = UserBotInstancesRunning_form.avg_tool_bots_running
...
def resume_bot(request):
    ...
    data = fetch_snapshot_bot_detail(uuid, us, port, SupportMultiBoxConsumer.BOT_ALIAS)
    if not data:
        html = '<h1>Bot running or not found snapshot</h1>'
        return HttpResponse(html)
    context = {'apis': [data['api_name']], 'user': us, 'id': uuid, 'data': data, 'resume': True,
               'bot_alias': SupportMultiBoxConsumer.BOT_ALIAS, 'ws_define': 'ws_support_multi_box'}
    return render(request, 'support_multi_box/index.html', context)
...
def snapshot(request):
    ...
    base_srv = BaseService()
    port = request.META['SERVER_PORT']
    list_bot_snapshot = fetch_snap_shot_bot(us, port, SupportMultiBoxConsumer.BOT_ALIAS)
    data = {'srv_id': base_srv.srv_id, 'bot_alias': SupportMultiBoxConsumer.BOT_ALIAS, 'suff_ws': 'ws_support_multi_box',
            'bot_name': 'Support Multi Boxes Bot', 'title': 'Support Multi Boxes Bot', 'bot_url': 'support-multi-box', 'list_bot_snapshot': list_bot_snapshot}
    return render(request, 'snapshot_simple.html', data)


@login_required()
def HowToUseSupportMultyBox(request):
    data = {"usage":"data"}
    return render(request, 'support_multi_box/how_to_use_SupportMultiBox.html', data)
...
def plot_chart(request):
    ...
    if not uuid:
        # Get list bot
        list_bot_chart = fetch_allbot_with_snapshot(us, port, SupportMultiBoxConsumer.BOT_ALIAS)
        data = {'bot_name': 'Support Multi Boxes Bot', 'title': 'Support Multi Boxes Bot', 'bot_url': 'support-multi-box',
                'list_bot_chart': list_bot_chart}
        return render(request, 'chart_tracking.html', data)
    # fetch data of bot, show chart
    bot_config = fetch_snapshot_bot_detail(uuid, us, port, SupportMultiBoxConsumer.BOT_ALIAS, True)
    if not bot_config:
