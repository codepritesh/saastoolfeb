...
from .consumers import TwoWaySpToolConsumer
...
def index(request, id=''):
    ...
    context = {'apis': apis, 'user': us, 'bot_alias': TwoWaySpToolConsumer.BOT_ALIAS, 'ws_define': 'ws_two_way_sp_tool'}
    if id:
        base_srv = BaseService()
        bot_alias = TwoWaySpToolConsumer.BOT_ALIAS
        data = base_srv.fetch_detail(bot_alias, str(id))
        if data:
            apis = [data.get('api_name')]
            context.update({'apis': apis, 'id': id, 'user': us, 'data': data, 'bot_alias': TwoWaySpToolConsumer.BOT_ALIAS})
    ...
    if user_permission_data.two_way_sp_tool:
        try: 
        ...
        return render(request, 'two_way_sp/index.html', context)
    else:
        messages.error(request, ('You are not allowed to see two_way_sp_tool page please subscribe'))
...
def tracking(request):
    ...
    data = {'tracking_user':tracking_user,'srv_id': base_srv.srv_id, 'bot_alias': TwoWaySpToolConsumer.BOT_ALIAS, 'suff_ws': 'ws_two_way_sp_tool',
            'bot_name': 'Two Way Sp Bot', 'title': 'Two Way Sp Bot', 'bot_url': 'two_way_sp_tool'}
    try:
        user_id = request.user.id  
        UserBotInstancesRunning_form = UserBotInstancesRunning.objects.get(pk=user_id)
        bot_one = UserBotInstancesRunning_form.avg_tool_bots_running
...
def resume_bot(request):
    ...
    data = fetch_snapshot_bot_detail(uuid, us, port, TwoWaySpToolConsumer.BOT_ALIAS)
    if not data:
        html = '<h1>Bot running or not found snapshot</h1>'
        return HttpResponse(html)
    context = {'apis': [data['api_name']], 'user': us, 'id': uuid, 'data': data, 'resume': True,
               'bot_alias': TwoWaySpToolConsumer.BOT_ALIAS, 'ws_define': 'ws_two_way_sp_tool'}
    return render(request, 'two_way_sp/index.html', context)
...
def snapshot(request):
    ...
    base_srv = BaseService()
    port = request.META['SERVER_PORT']
    list_bot_snapshot = fetch_snap_shot_bot(us, port, TwoWaySpToolConsumer.BOT_ALIAS)
    data = {'srv_id': base_srv.srv_id, 'bot_alias': TwoWaySpToolConsumer.BOT_ALIAS, 'suff_ws': 'ws_two_way_sp_tool',
            'bot_name': 'Two Way Sp Bot', 'title': 'Two Way Sp Bot', 'bot_url': 'two_way_sp_tool', 'list_bot_snapshot': list_bot_snapshot}
    return render(request, 'snapshot_simple.html', data)


@login_required()
def how_to_use_TwoWaySpBot(request):
    data = {"usage":"data"}
    return render(request, 'two_way_sp/how_to_use_TwoWaySpBot.html', data)
...
def plot_chart(request):
    ...
    if not uuid:
        # Get list bot
        list_bot_chart = fetch_allbot_with_snapshot(us, port, TwoWaySpToolConsumer.BOT_ALIAS)
        data = {'bot_name': 'Two Way Sp Bot', 'title': 'Two Way Sp Bot', 'bot_url': 'two_way_sp_tool',
                'list_bot_chart': list_bot_chart}
        return render(request, 'chart_tracking.html', data)
    # fetch data of bot, show chart
    bot_config = fetch_snapshot_bot_detail(uuid, us, port, TwoWaySpToolConsumer.BOT_ALIAS, True)
    if not bot_config:
