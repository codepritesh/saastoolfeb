...
from .consumers import SupportTrailingStopConsumer
...
def index(request, id=''):
    ...
    context = {'apis': apis, 'user': us, 'bot_alias': SupportTrailingStopConsumer.BOT_ALIAS, 'ws_define': 'support_trailing_stop'}
    if id:
        base_srv = BaseService()
        bot_alias = SupportTrailingStopConsumer.BOT_ALIAS
        data = base_srv.fetch_detail(bot_alias, str(id))
        if data:
            apis = [data.get('api_name')]
            context.update({'apis': apis, 'id': id, 'user': us, 'data': data, 'bot_alias': SupportTrailingStopConsumer.BOT_ALIAS})
    ...
    if user_permission_data.support_trailing_stop_bot:
        ...
        return render(request, 'support_trailing_stop/index.html', context)
    else:
        messages.error(request, ('You are not allowed to see support_trailing_stop_bot page please subscribe'))
...
def tracking(request):
    ...
    data = {'tracking_user':tracking_user,'srv_id': base_srv.srv_id, 'bot_alias': SupportTrailingStopConsumer.BOT_ALIAS, 'suff_ws': 'support_trailing_stop',
            'bot_name': 'Support Trailing Stop', 'title': 'Support Trailing Stop', 'bot_url': 'support-trailing-stop-bot'}
    try:
        user_id = request.user.id  
        UserBotInstancesRunning_form = UserBotInstancesRunning.objects.get(pk=user_id)
        bot_one = UserBotInstancesRunning_form.avg_tool_bots_running
...
def resume_bot(request):
    ...
    data = fetch_snapshot_bot_detail(uuid, us, port, SupportTrailingStopConsumer.BOT_ALIAS)
    if not data:
        html = '<h1>Bot running or not found snapshot</h1>'
        return HttpResponse(html)
    context = {'apis': [data['api_name']], 'user': us, 'id': uuid, 'data': data, 'resume': True,
               'bot_alias': SupportTrailingStopConsumer.BOT_ALIAS, 'ws_define': 'support_trailing_stop'}
    return render(request, 'support_trailing_stop/index.html', context)
    
@login_required()
def HowToUseSupportTrailingBot(request):
    data = {"usage":"data"}
    return render(request, 'support_trailing_stop/how_to_use_SupportTrailingBot.html', data)

def snapshot(request):
    us = str(request.user)
    base_srv = BaseService()
    port = request.META['SERVER_PORT']
    list_bot_snapshot = fetch_snap_shot_bot(us, port, SupportTrailingStopConsumer.BOT_ALIAS)
    data = {'srv_id': base_srv.srv_id, 'bot_alias': SupportTrailingStopConsumer.BOT_ALIAS, 'suff_ws': 'support_trailing_stop',
            'bot_name': 'Support Trailing Stop', 'title': 'Support Trailing Stop', 'bot_url': 'support-trailing-stop-bot', 'list_bot_snapshot': list_bot_snapshot}
