...
from .consumers import AtrTrailingStopConsumer
from admin_dashboard.models import UserBotInstancesRunning
...
modes = [{'key': 'Simple', 'value': 'simple'}, {'key': 'Divide And Conrque', 'value': 'divide_and_conrque'}, {'key': 'Trailing Stop', 'value': 'trailing_stop'}]
...
def index(request, id=''):
    ...
    option = modes.copy()
    context = {'apis': apis, 'user': us, 'bot_alias': AtrTrailingStopConsumer.BOT_ALIAS, 'ws_define': 'ws_atr_trailing_stop_bot', 'modes': option}
    if id:
        base_srv = BaseService()
        bot_alias = AtrTrailingStopConsumer.BOT_ALIAS
        data = base_srv.fetch_detail(bot_alias, str(id))
        option = fetch_mode_display(modes, data['mode'])
        if data:
            apis = [data.get('api_name')]
            context.update({'apis': apis, 'id': id, 'user': us, 'data': data, 'bot_alias': AtrTrailingStopConsumer.BOT_ALIAS, 'modes': option})
    ...
    if user_permission_data.support_trailing_stop_bot:
        try:
        ...
        return render(request, 'atr_trailing_stop/index.html', context)
    else:
        messages.error(request, ('You are not allowed to see atr_trailing_stop page please subscribe'))
...
def tracking(request):
    ...
    data = {'tracking_user':tracking_user,'srv_id': base_srv.srv_id, 'bot_alias': AtrTrailingStopConsumer.BOT_ALIAS, 'suff_ws': 'ws_atr_trailing_stop_bot',
            'bot_name': 'ATR Trailing Stop Bot', 'title': 'ATR Trailing Stop Bot', 'bot_url': 'atr-trailing-stop-bot'}
    try:
        user_id = request.user.id
        UserBotInstancesRunning_form = UserBotInstancesRunning.objects.get(pk=user_id)
        bot_one = UserBotInstancesRunning_form.avg_tool_bots_running
...
def resume_bot(request):
    ...
    data = fetch_snapshot_bot_detail(uuid, us, port, AtrTrailingStopConsumer.BOT_ALIAS)
    if not data:
        html = '<h1>Bot running or not found snapshot</h1>'
        return HttpResponse(html)
    context = {'apis': [data['api_name']], 'user': us, 'id': uuid, 'data': data, 'resume': True,
               'bot_alias': AtrTrailingStopConsumer.BOT_ALIAS, 'ws_define': 'ws_atr_trailing_stop_bot'}
    return render(request, 'atr_trailing_stop/index.html', context)
...
def snapshot(request):
    ...
    base_srv = BaseService()
    port = request.META['SERVER_PORT']
    list_bot_snapshot = fetch_snap_shot_bot(us, port, AtrTrailingStopConsumer.BOT_ALIAS)
    data = {'srv_id': base_srv.srv_id, 'bot_alias': AtrTrailingStopConsumer.BOT_ALIAS, 'suff_ws': 'ws_atr_trailing_stop_bot',
            'bot_name': 'ATR Trailing Stop Bot', 'title': 'ATR Trailing Stop Bot', 'bot_url': 'atr-trailing-stop-bot', 'list_bot_snapshot': list_bot_snapshot}
...
def plot_chart(request):
    ...
    if not uuid:
        # Get list bot
        list_bot_chart = fetch_allbot_with_snapshot(us, port, AtrTrailingStopConsumer.BOT_ALIAS)
        data = {'bot_name': 'ATR Trailing Stop Bot', 'title': 'ATR Trailing Stop Bot', 'bot_url': 'atr-trailing-stop-bot',
                'list_bot_chart': list_bot_chart}
        return render(request, 'chart_tracking.html', data)
    # fetch data of bot, show chart
    bot_config = fetch_snapshot_bot_detail(uuid, us, port, AtrTrailingStopConsumer.BOT_ALIAS, True)
    if not bot_config:
